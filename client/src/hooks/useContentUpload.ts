  ...
  return useMutation({
    mutationFn: async (data: WebSeriesData) => {
      ...
      for (const seasonData of data.seasons) {
        ...
        // Then, create the season with episode IDs
      ...
      // Finally, create the upload_content record
      const { data: uploadResult, error: uploadError } = await supabase
        .from('upload_content')
        .insert([{
          title: data.title,
...
export const useUploadShow = () => {
  ...
  return useMutation({
    mutationFn: async (data: ShowData) => {
      ...
      // Then, create the show record with episode IDs
